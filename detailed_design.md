...
- **下单流程**：用户点击“结算” → CartController → CartService → OrderService → OrderRepository → 返回订单信息 → 前端显示  
- **商品管理流程（管理员）**：AdminController → ProductService → ProductRepository → 返回商品状态 → 前端显示  
...
| 配送中 (Shipping) | 商家发货 | 管理员标记发货 |
| 已完成 (Completed) | 用户确认收货 | 用户点击“确认收货” |
| 已取消 (Cancelled) | 用户取消或支付失败 | 用户取消/支付超时 |
